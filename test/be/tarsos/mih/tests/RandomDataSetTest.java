...
public class RandomDataSetTest {
	...
	static int dataSetSize = 100000;
	static int numBits = 64;
	static int longs = numBits/64;
	static int maxHammingDistance = 8;
	...
	public static void oneTimeSetUp() {
		...
		for(int i = 0 ; genQueries < numQueries ; i++){
			for(int k = 0 ; k < r.nextInt(numNearNeighboursToGenerate) + 1 ; k++){
				...
				int hammingDistance = 8;//r.nextInt(maxHammingDistance);
	...
	private static void  createOrReadDataset(){
		...
		//mih = new MultiIndexHasher(numBits, maxHammingDistance, 4,new MapDBStorage(4,"data.db"));
	...
	public void testRandomDataSetMIH(){		
		for(int k =0 ; k < queries.size() ; k++){
			...
			for(BitSetWithID result: results){
				...
				/*if(query.getIdentifier() != result.getIdentifier()){
					System.out.println("******************************");
					System.out.println(query.getIdentifier());
					results.stream().map(BitSetWithID::getIdentifier).forEach(System.out::println);

					System.out.println(query.hammingDistance(result));
